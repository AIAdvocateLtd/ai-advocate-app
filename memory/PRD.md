...
## What's Implemented (May 2026 — MVP v1.3)

### 🔐 Authentication ✅ HARDENED v1.3
- **Email/password** (JWT HS256, 30-day, bcrypt-12)
- **Real Google OAuth** — server-side ID token verification via `google-auth` library when `GOOGLE_CLIENT_ID` is set in backend .env. Falls back to demo path otherwise.
- **Sign in with Apple** — verifies Apple identity_token JWT against Apple's JWKS endpoint. Requires `APPLE_SERVICES_ID`, `APPLE_TEAM_ID`, `APPLE_KEY_ID`, `APPLE_PRIVATE_KEY` env vars from Apple Developer Portal.
- `GET /api/auth/providers` — frontend queries this to know which social buttons to show.
- All paths issue an internal AI Advocate JWT after verification.

### 💳 Payments ✅ HARDENED v1.3
- Stripe Checkout (£14.99/mo or £119.99/yr) with 14-day trial gating.
- **Stripe Webhook** at `/api/webhook/stripe` — handles `checkout.session.completed` (activates), `customer.subscription.deleted` (cancels), `customer.subscription.updated` (status sync), `invoice.payment_failed`. Signature verified via `STRIPE_WEBHOOK_SECRET` env var.

### 📄 Multilingual PDFs ✨ NEW v1.3
- All 11 languages render correctly in PDF, including Arabic, Urdu (RTL), Chinese, Hindi.
- NotoSans + NotoSansArabic + NotoSansSC + NotoSansDevanagari fonts embedded in `/app/backend/fonts/`.
- `_font_for_lang()` picks the correct font variant per language code.
- Branded footer on every page: "Generated by AI ADVOCATE / AI lawyer in your pocket — get yours in 60s" + QR code linking to `APP_PUBLIC_URL?ref=pdf` for viral marketing.

### Dashboard — Heraldic Crest Icons (no boxes) ✨ v1.2
